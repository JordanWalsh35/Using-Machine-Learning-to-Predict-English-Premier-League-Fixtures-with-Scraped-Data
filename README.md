...
<p align="justify"> The dataframes were merged to make categorical (team) encoding simpler. Given that in each season three teams are relegated and promoted, if we did encoding for each season individually (numbering the teams 1 to 20 so that our machine learning model can use this categorical data) we would have some inconsistencies and our model would think two different teams were the same. For example, if Wolves were encoded with the number 20 in one season but got relegated for the next season, the replacement (promoted) team would then get the code 20 and our model would think this was the same team.   </p>
